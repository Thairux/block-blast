...
export function breakLines(board: Board): { lines: number, gems: number } {
  "worklet";
  const boardLength = board.length;
  const rowsToClear = new Set<number>();
  const colsToClear = new Set<number>();
  ...
  let gems = 0;
  const lines = rowsToClear.size + colsToClear.size;

  if (lines > 0) {
    rowsToClear.forEach((row) => {
      for (let col = 0; col < boardLength; col++) {
        if (board[row][col].hasGem) gems++;
    ...
    colsToClear.forEach((col) => {
      for (let row = 0; row < boardLength; row++) {
        // If it was already cleared by a row, don't count gem twice
        if (board[row][col].blockType !== BoardBlockType.EMPTY) {
            if (board[row][col].hasGem) gems++;
            board[row][col].blockType = BoardBlockType.EMPTY;
            board[row][col].hasGem = false;
        }
